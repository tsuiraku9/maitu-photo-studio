...
`MaiTu 写真` 是面向 MaiBot 的异步真实手机照片插件（Manifest ID：`maitu.photo-studio`）。它将 Planner 的一次“发张照片”请求转为可持久化、可追踪的后台任务：依据聊天语境选择合适的生图工具，复用或生成参考图，调用图片服务，并在成功后把结果投递回原聊天。

## 插件介绍

插件将“像真人一样发一张照片”拆成可控的几个环节：

- **照片生成与投递**：含 bot 本人的写真和不含人物的环境/物品照走不同工具；请求先入队，成功后才发送图片。
- **参考图库**：人物参考板用于稳定身份；服装与合格的私密室内场景参考板用于稳定穿搭和地点。所有参考图都经过压缩、标签校验和状态管理。
- **聊天连续性**：同一群聊或私聊流在相同场景、同一自然日和有效时长内，优先复用上次的服装与场景参考；管理员可以固定或重置。
- **自动补库**：人物写真在成功投递后，能够从结果图异步补充本次缺失的服装或场景参考，不会阻塞用户的照片投递。
- **任务可追踪**：每次工具调用都会返回 `task_id`；队列、结果、实际使用的参考图和投递状态都会持久化，可在重载后继续查看或处理。
- **队列与计费保护**：排队任务可在重载后恢复；已经发起付费图片请求却没有保存结果的运行中任务不会自动重试，以避免重复计费。
