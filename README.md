# 麦麦写真 · 让麦麦更真实地发送照片
>
> **⚠️ 使用声明**：本插件借助 AI 辅助开发，仍可能存在未发现的缺陷、边界情况或非预期行为。启用前请审阅源码，并结合自身环境评估风险；使用本插件产生的风险与后果，请由使用者自行承担。
>
> **💳 费用提示**：启用照片工具调用建议、自动补库，或高频使用图片生成功能，均可能增加模型调用次数和服务商费用。请结合服务商的价格与配额规则，合理控制调用频率与使用范围。
...
自动补库只处理含人物写真中缺失的**服装**和**场景**参考，不会自动生成人物参考板。它始终发生在图片确认投递之后，因此参考图模型慢、失败或待审核都不会阻塞已经完成的照片。场景补库会复用生成前的场景文字资格判断；判断通过后立即创建场景补库任务。补库完成得较晚时，新参考图仍会入库；但只有来源照片仍是该聊天当前、同场景的连续性记录时，才会写回连续性，避免旧任务覆盖较新的照片状态。
